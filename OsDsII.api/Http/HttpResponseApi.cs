...
namespace OsDsII.api.Http
{
    public class  HttpResponseApi<T> where T : class
    {
        ...
        {
            var httpResponse = new HttpResponseApi<T>(data,"ok", HttpStatusCode.OK);
            return new ObjectResult(httpResponse) { StatusCode = httpResponse.StatusCode };
        }
        public static IActionResult Created(T data)
        {
            var httpResponse = new HttpResponseApi<T>(data, "Created sucecessfully", HttpStatusCode.Created);
            return new ObjectResult(httpResponse) { StatusCode=httpResponse.StatusCode }; 
        }
